...
open class FSPageControl: UIControl {
    ...
    open override func layoutSublayers(of layer: CALayer) {
        ...
        for (index,_) in self.indicatorLayers.enumerated() {
            if (selectedIndex < 0) {
                selectedIndex = (index == self.currentPage) ? index : selectedIndex
            }
        }
        ...
        for (index,value) in self.indicatorLayers.enumerated() {
            ...
            let pathSize = path?.bounds.size ?? CGSize(width: diameter, height: diameter)

            var origin: CGPoint;
            let selectedSize = image != nil ? size : pathSize

            let padding = selectedSize.width + spacing + 5
            let originPadding = CGPoint(x: (x - (size.width-diameter)*0.5) + padding, y: self.contentView.bounds.midY-size.height*0.5)
            let originNormal = CGPoint(x: (x - (size.width-diameter)*0.5), y: self.contentView.bounds.midY-size.height*0.5)

            if (selectedIndex == 0) {
                if (index > 0) {
                    origin = originPadding
                } else {
                    origin = originNormal
                }
            } else {
